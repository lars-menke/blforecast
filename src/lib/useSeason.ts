...
function simulateSeason(
  ...
  for (let s = 0; s < SIMULATIONS; s++) {
    ...
    for (const m of remaining) {
      const r = Math.random();
      if (r < m.pH) { pts[m.home] = (pts[m.home] ?? 0) + 3; gd[m.home] = (gd[m.home] ?? 0) + 1; gd[m.away] = (gd[m.away] ?? 0) - 1; }
      else if (r < m.pH + m.pD) { pts[m.home] = (pts[m.home] ?? 0) + 1; pts[m.away] = (pts[m.away] ?? 0) + 1; }
      else { pts[m.away] = (pts[m.away] ?? 0) + 3; gd[m.away] = (gd[m.away] ?? 0) + 1; gd[m.home] = (gd[m.home] ?? 0) - 1; }
    }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Find remaining matches and calculate probabilities
